...
type BackendErrorObject = {
  ...
  errors?: string[];
  details?: { message?: string }[] | ZodFlattenError;
};
...
export function normalizeApiError(
  ...
  if (error instanceof AxiosError) {
    ...
    if (typeof data === "object") {
      ...
      // Zod flatten() error in error field
      ...
      // Zod flatten() error in details field
      if (payload.details && typeof payload.details === "object" && !Array.isArray(payload.details)) {
        const zodError = payload.details as ZodFlattenError;
