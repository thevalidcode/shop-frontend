...
export default function PaymentMethodActions({
  ...
  const handleSave = async (updated: PaymentGateway) => {
    ...
    const usdMin = convert(userCurrency, "USD", updated.min || "0").amount;
    const usdMax = convert(userCurrency, "USD", updated.max || "0").amount;
    const response = await mutation({ 
      ...updated, 
      min: usdMin, 
      max: usdMax,
      description: updated.description || undefined,
      secretKey: updated.secretKey || undefined
    });
