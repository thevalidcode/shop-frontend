...
export default function AdminBlogsPage() {
  ...
  const handleDeleteSingle = (shopScopedId: number) => {
    setDeleteIds([shopScopedId]);
    setDeleteOpen(true);
  };

  const handleDeleteConfirm = () => {
    const usersUids = filteredBlogs
      .filter((u) => deleteIds.includes(u.shopScopedId))
      .map((u) => u.uid);
    deleteMultipleBlogs({ uids: usersUids });
    setBlogs((prev) => prev.filter((u) => !deleteIds.includes(u.shopScopedId)));
    setDeleteIds([]);
  };
  ...
  const handleSave = (updated: Blog) => {
    setBlogs((prev) =>
      isEditing
        ? prev.map((b) =>
            b.shopScopedId === updated.shopScopedId ? updated : b,
          )
        : [updated, ...prev],
    );
                  ...
                  <div className="mt-auto flex items-center justify-between text-xs text-gray-500">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button
                        size="icon"
                        variant="destructive"
                        onClick={() => handleDeleteSingle(blog.shopScopedId)}
