...
export default function UsersPage() {
  ...
  const selectAllCurrent = (checked: boolean) => {
    if (checked) setSelected(current.map((u) => u.shopScopedId));
    else setSelected([]);
  };
  ...
  const handleDeleteConfirm = () => {
    ...
    setUsers((prev) =>
      prev.filter((u) => !deleteIds.includes(u.shopScopedId))
    );
  ...
  const handleToggleBan = (shopScopedId: number) => {
    let updatedUser: UpdateUserByAdminProps | null = null;

    setUsers((prev) =>
      prev.map((u) => {
        if (u.shopScopedId !== shopScopedId) return u;
  ...
  const handleBanSelectedUsers = (
    ...
    setUsers((prev) =>
      prev.map((u) => {
        if (!selectedIds.includes(u.shopScopedId)) return u;
