...
export default function UserCardList({
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {users.map((u) => (
        <Card key={u.shopScopedId} className="rounded-2xl overflow-hidden">
          <CardHeader className="flex items-start gap-3 p-4">
            <Checkbox
              checked={selected.includes(u.shopScopedId)}
              onCheckedChange={(v) => onSelect(u.shopScopedId, v as boolean)}
            />
            <img
          ...
          <CardFooter className="flex justify-between items-center p-4">
            <div className="flex items-center gap-2">
              ...
              <Button
                size="sm"
                variant="outline"
                onClick={() => onToggleBan(u.shopScopedId)}
              >
