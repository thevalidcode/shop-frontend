...
import { useUserVerifySessionCode } from "@/hooks/use-user";
...
export default function Signup() {
          ...
          <CardContent className="space-y-8 p-6 sm:p-8">
            <div className="text-center">
              <p className="text-destructive">
                Shop configuration not found. Please try again later.
              </p>
            </div>
          </CardContent>
        </Card>
  ...
  useEffect(() => {
    ...
    if (!shopId || !domain) return; // Wait for shop context to be ready
  ...
  }, [sessionCodeFromQuery, verifySessionCode, shopId, domain]);
  ...
  const handleGoogleLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    router.push(
      `https://auth.validpanel.com/api/auth/shop/google?shopId=${shopId}&redirect=https://${domain}/auth/signup`,
    );
  };
  ...
  return (
    <div className="flex items-center justify-center px-2 py-25 min-h-screen">
      <Card className="w-full max-w-md shadow-xl mx-auto">
        <CardContent className="space-y-8 p-6 sm:p-8">
          {/* Signup Form */}
